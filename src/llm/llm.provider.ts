...
import { getProxyParams } from '../utils';
import { CONDENSE_PROMPT, QA_PROMPT } from './promts';
...
export class LLM {
  ...
  private async initChroma(collectionName: string) {
    ...
    const embeddings = new OpenAIEmbeddings({}, params);
  ...
  private createChain = async (vectorStore: Chroma) => {
    ...
    const docChain = loadQAChain(model, {
      type: 'stuff',
      prompt: QA_PROMPT,
    });
...
export const LLMProvider: Provider<LLM> = {
  ...
  useFactory: (configService: ConfigService<Config>) => {
    ...
    return new LLM(proxyPath, apiKey, dbPath, enableProxy);
  },
};
